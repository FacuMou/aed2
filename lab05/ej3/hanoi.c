...
hanoi_t hanoi_init(unsigned int disk_count) {
    ...
    assert(hanoi != NULL);
    hanoi->source = stack_empty(); //Era necesario inicializar
    hanoi->aux = stack_empty();
    hanoi->target = stack_empty(); // No se debia inicializar con NULL
...
hanoi_t hanoi_destroy(hanoi_t hanoi) {
    assert(hanoi != NULL);
    stack_destroy(hanoi->source); // Era necesario destruir individualmente el stack de cada parte
